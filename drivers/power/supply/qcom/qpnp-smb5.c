...
/*modified by yangshengtao 2018.11.12 add system level interface
		for thermal control battery charging currents start*/
	POWER_SUPPLY_PROP_SYSTEM_TEMP_LEVEL,
/*modified by yangshengtao 2018.11.12 add system level interface
		for thermal control battery charging currents end*/
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_FORCE_RECHARGE:
		val->intval = 0;
		break;
/*2018/09/25 add charge_full_design  sysfs interface to battery, begin*/
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
/*2018/11/29 change this property to a fixed value as customer require , begin*/
#if 1
		val->intval = 4000;
#else
		rc = smblib_get_prop_from_bms(chg,
...
#endif
/*2018/11/29 change this property to a fixed value as customer require , end*/
		break;
/*2018/09/25 add charge_full_design  sysfs interface to battery, begin*/
...
static int smb5_init_hw(struct smb5 *chip)
{
	 ...
	 */
	//modified by yangshengtao re-run apsd when smbcharger driver init  start
	if ((chg->smb_version == PMI632_SUBTYPE)
		|| (chg->smb_version == PM6150_SUBTYPE)) {
		schgm_flash_init(chg);
		smblib_rerun_apsd_if_required(chg);
	}
	//modified by yangshengtao re-run apsd when smbcharger driver init	end
